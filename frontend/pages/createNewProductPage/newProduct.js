...
document.getElementById("create-product-btn").addEventListener('click', (event)=>{
    ...
    getUser().then(user => {
        ...
        if(selectedTypeValue === "clothes") {
            ...
            const weight = document.getElementById("weight").value;
            console.log(suppType, weight)
            bodyHandle = `email=${user.email}&selectedTypeValue=${selectedTypeValue}&name=${name}&image=${image}&price=${price}&desc=${desc}&suppType=${suppType}&weight=${weight}`;
        } else {
            const equipType = document.getElementById("equip-type").value;
            bodyHandle = `email=${user.email}&selectedTypeValue=${selectedTypeValue}&name=${name}&image=${image}&price=${price}&desc=${desc}&equipType=${equipType}`;
        }
